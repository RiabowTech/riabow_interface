...
import { PrimitApp } from "./elements/page-objects";
...
export const test = baseTest.extend<{
  ...
  context: async ({ appUrl }, use) => {
    ...
    console.log("[Preparing Primit App]");
    const page = await ctx.newPage();
    await page.goto(appUrl);
    const primit = new PrimitApp(page, wallet, appUrl);
    await primit.header.connectWallet();
    await primit.closeAllToasts();
  ...
  appUrl: async ({}, use) => use(process.env.PRIMIT_BASE_URL || "https://app.primit.io"),

  primit: async ({ wallet, page, appUrl }, use) => {
    const primit = new PrimitApp(page, wallet, appUrl);
    await use(primit);
  },
});
