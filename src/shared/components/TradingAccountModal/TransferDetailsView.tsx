...
export const TransferDetailsView = () => {
  ...
  const [, setTradingAccountDepositViewChain] = useTradingAccountDepositViewChain();
  const [, setTradingAccountWithdrawalViewChain] = useTradingAccountWithdrawalViewChain();
  const [, setTradingAccountDepositViewTokenAddress] = useTradingAccountDepositViewTokenAddress();
  const [, setTradingAccountDepositViewTokenInputValue] = useTradingAccountDepositViewTokenInputValue();
  const [, setTradingAccountWithdrawalViewTokenAddress] = useTradingAccountWithdrawalViewTokenAddress();
  const [, setTradingAccountWithdrawalViewTokenInputValue] = useTradingAccountWithdrawalViewTokenInputValue();
  ...
  const handleRepeatTransaction = () => {
    ...
    if (selectedTransfer.operation === "deposit") {
      setTradingAccountDepositViewChain(selectedTransfer.sourceChainId as SourceChainId);
      setTradingAccountDepositViewTokenAddress(selectedTransfer.token);
      setTradingAccountDepositViewTokenInputValue(formatAmountFree(selectedTransfer.sentAmount, token.decimals));
      setTradingAccountModalOpen("deposit");
      return;
    }

    if (selectedTransfer.operation === "withdrawal") {
      setTradingAccountWithdrawalViewChain(selectedTransfer.sourceChainId as SourceChainId);
      setTradingAccountWithdrawalViewTokenAddress(selectedTransfer.token);
      setTradingAccountWithdrawalViewTokenInputValue(formatAmountFree(selectedTransfer.sentAmount, token.decimals));
      setTradingAccountModalOpen("withdraw");
    }
  };
