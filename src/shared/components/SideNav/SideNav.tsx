    ...
    <button className={cx("group cursor-pointer select-none py-1", { "w-full": !isCollapsed })} onClick={onClick}>
      <div
        className={cx(
          ...
          {
            "bg-[#262626] text-[#D4A574]": isActive,
      ...
      >
        <div
          className={cx(
            ...
            isActive ? "text-[#D4A574]" : "text-slate-300 group-hover:text-[#D4A574]"
          )}
        >
          {icon}
        </div>
        <div className={cx("flex items-center gap-6", { hidden: isCollapsed })}>
          <span
            className={cx(
              ...
              isActive ? "text-[#D4A574]" : "text-slate-200"
            )}
          >
            {label}
          </span>
        ...
        {isActive ? (
          <div className="pointer-events-none absolute right-1 top-2 bottom-2 w-[3px] rounded-full bg-[#D4A574] shadow-[0_0_10px_rgba(203,213,225,0.9)]" />
        ) : null}
      </div>
    </button>
  );
