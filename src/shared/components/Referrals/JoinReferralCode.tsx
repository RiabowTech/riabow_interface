...
import { type MultichainAction, MultichainActionType } from "@/modules/lighter/domain/multichain/codecs/CodecUiHelper";
import { getMultichainTransferSendParams } from "@/modules/lighter/domain/multichain/getSendParams";
import { estimateMultichainDepositNetworkComposeGas } from "@/modules/lighter/domain/multichain/useMultichainDepositNetworkComposeGas";
...
import { toastCustomOrStargateError } from "components/TradingAccountModal/toastCustomOrStargateError";
...
function ReferralCodeFormMultichain({
  ...
  return (
    <form onSubmit={buttonState.onSubmit} className="flex flex-col gap-15">
      ...
      <Button variant="primary-action" type="submit" className="w-full" disabled={buttonState.disabled}>
        {buttonState.text}
      </Button>
    </form>
  );
