/**
 * Zanbara UI Components Guidelines（Figma）文件与章节节点对照。
 * 用于 Storybook Design 插件、文档链接；node-id 使用 URL 中的连字符形式。
 */

export const FIGMA_ZANBARA_GUIDELINES_FILE_KEY = "Xy4iA3ZnzAZPmOduAyqmtG";

const BASE = `https://www.figma.com/design/${FIGMA_ZANBARA_GUIDELINES_FILE_KEY}/Primit-UI-Components-Guidelines`;

/** 构建带 Dev Mode 的稿面链接 */
export function zanbaraGuidelinesFigmaUrl(nodeIdDash: string): string {
  return `${BASE}?node-id=${nodeIdDash}&m=dev`;
}

/** 第 11 章 Popup_PC 主 frame（`228-3282`）— 对应 `Modal` 的 `variant="zanbara"` */
export const ZANBARA_MODAL_POPUP_PC_FIGMA_URL = zanbaraGuidelinesFigmaUrl("228-3282");

/** 与仓库内已嵌入章节 / Story 对应的 Figma 节点（精确到 frame/section） */
export const zanbaraChapterFigmaNode: Record<
