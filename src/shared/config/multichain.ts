...
import { LayerZeroEndpointId } from "@/modules/lighter/domain/multichain/types";
import {
  AnyChainId,
  ARBITRUM,
  ARBITRUM_SEPOLIA,
...
// In development mode, allow ARBITRUM_SEPOLIA to be used as a source chain for same-chain deposits
if (isDevelopment()) {
...
for (const tokenSymbol in TOKEN_GROUPS) {
  for (const chainIdString in TOKEN_GROUPS[tokenSymbol]) {
    ...
    for (const sourceChainIdString in TOKEN_GROUPS[tokenSymbol]) {
      ...
      // Allow same chain deposits in development mode or when DEBUG_MULTICHAIN_SAME_CHAIN_DEPOSIT is enabled
      if (!isDevelopment() && !DEBUG_MULTICHAIN_SAME_CHAIN_DEPOSIT && (settlementChainId as number) === (sourceChainId as number)) {
        continue;
      }
...
// In development mode, allow ARBITRUM_SEPOLIA to be used as a source chain for multicall
if (isDevelopment()) {
 ...
 * Source contract lives in the protocol synthetics repository.
 */
