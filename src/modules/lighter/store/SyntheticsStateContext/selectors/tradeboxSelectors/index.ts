...
import { SyntheticsState } from "@/modules/lighter/store/SyntheticsStateContext/SyntheticsStateContextProvider";
import { createSelector } from "@/modules/lighter/store/SyntheticsStateContext/utils";
import {
...
export const selectTradeboxIsFromTokenTradingAccount = (s: SyntheticsState) => s.tradebox.isFromTokenTradingAccount;
...
const selectNextValuesForIncrease = createSelector(
  (q): Parameters<typeof makeSelectNextPositionValuesForIncrease>[0] => {
    ...
    const isFromTokenTradingAccount = q(selectTradeboxIsFromTokenTradingAccount);

    const externalSwapQuote = q(selectExternalSwapQuote);
    const isExpressTxn =
      isFromTokenTradingAccount ||
      (fromTokenAddress !== NATIVE_TOKEN_ADDRESS && q(selectIsExpressTransactionAvailable));
...
export const selectTradeboxFromToken = createSelector((q): TokenData | undefined => {
  ...
  if (isFromTokenTradingAccount && !token.isTradingAccount) {
    return {
      ...token,
      isTradingAccount: true,
      balance: token.tradingAccountBalance,
    };
  } else if (!isFromTokenTradingAccount && token.isTradingAccount) {
    return {
      ...token,
...
export const selectTradeboxPayTokenAllowance = createSelector((q) => {
  if (q(selectTradeboxIsFromTokenTradingAccount)) {
    return maxUint256;
  }
