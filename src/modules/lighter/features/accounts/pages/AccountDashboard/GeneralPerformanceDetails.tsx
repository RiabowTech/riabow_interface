...
import { useShowDebugValues } from "@/modules/lighter/store/SyntheticsStateContext/hooks/settingsHooks";
...
/**
 * Wrapper component that checks if Subsquid GraphQL is available.
 * If not available, renders empty state (will use centralized API in the future).
 */
export function GeneralPerformanceDetails({ chainId, account }: { chainId: number; account: Address }) {
  const client = getSubsquidGraphClient(chainId);

  // When Subsquid is not available, render empty state
  // TODO: Replace with centralized API data fetching in the future
  if (!client) {
    return <GeneralPerformanceDetailsEmpty />;
  }

  return <GeneralPerformanceDetailsWithData chainId={chainId} account={account} />;
}

/**
 * Empty state component when Subsquid GraphQL is not available
 */
function GeneralPerformanceDetailsEmpty() {
  return (
    <div className="overflow-hidden rounded-8" style={{ background: 'rgba(255, 255, 255, 0.05)' }}>
      <div className="border-b-1/2 border-slate-600 p-20 text-20 font-medium">
        <Trans>General Performance Details</Trans>
      </div>
      <div className="p-20 text-center text-gray-400">
        <Trans>Performance data is not available for this network</Trans>
      </div>
    </div>
  );
}
...
/**
 * Inner component that fetches and displays PnL summary data
 */
function GeneralPerformanceDetailsWithData({ chainId, account }: { chainId: number; account: Address }) {
