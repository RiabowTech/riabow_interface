...
import type { SortDirection } from "@/modules/lighter/context/SorterContext/types";
import {
...
export function LeaderboardAccountsTable({
  ...
  return (
    <div className="LeaderboardTable-panel rounded-b-8 bg-slate-900 w-full max-w-full overflow-hidden">
          ...
          <thead>
            <TableTheadTr className="text-body-medium">
              <TableHeaderCell
                title={t`Rank`}
                width={6}
                tooltip={t`Top traders ranked by realized PnL over the selected period.`}
                tooltipPosition="bottom-start"
              />
              <TableHeaderCell title={t`Address`} width={20} tooltipPosition="bottom-end" />
              <TableHeaderCell
                title={t`Realized PnL`}
                width={18}
                tooltip={t`Cumulative realized profit and loss from closed positions during the selected period. Unrealized PnL on open positions is not included.`}
              ...
              <TableHeaderCell
                title={t`Volume`}
                width={18}
                tooltip={t`Total trade notional (maker + taker) during the selected period.`}
                tooltipPosition="bottom-end"
                {...getSorterProps("volume")}
              />
              <TableHeaderCell
                title={t`Win/Loss`}
                width={12}
                tooltip={t`Wins and losses across closed positions during the selected period.`}
                tooltipPosition="bottom-end"
                {...getSorterProps("wins")}
              />
            </TableTheadTr>
      ...
      <TableTr hoverable={true} key={account.account}>
        ...
        <TableTd
          className={cx("numbers", {
            "text-typography-secondary": account.volume === 0n,
          })}
        >
          {account.volume ? formatUsd(account.volume) : "$\u200a0.00"}
        </TableTd>
...
const EmptyRow = memo(() => {
  return (
    <TableTr className="h-47">
      <TableTd colSpan={5} className="align-top text-typography-secondary">
