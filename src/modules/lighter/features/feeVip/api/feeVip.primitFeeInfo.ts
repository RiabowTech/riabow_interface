...
/**
 * Primit U 本位合约 `GET /api/v1/account/fee-info` 响应（与公开文档字段一致）。
 * @see https://developers.primit.io/zh-Hans/futures/usdt-margined/account/vip-tiers
 */
export interface PrimitFeeInfoFeeTierDto {
  level: number;
  label: string;
  maker: string;
  taker: string;
...
export interface PrimitFeeInfoProgressDto {
  next_level: number;
  next_label: string;
...
export interface PrimitFeeInfoDiscountsDto {
  referral: string;
  token_staking: string;
  multiplier: string;
}

export interface PrimitFeeInfoResponseDto {
  current_tier: number;
  ...
  volume_14d: string;
  volume_30d?: string;
  fee_tiers: PrimitFeeInfoFeeTierDto[];
  progress_to_next?: PrimitFeeInfoProgressDto | null;
  pending_tier?: number | null;
  pending_effective_at?: string | null;
  discounts?: PrimitFeeInfoDiscountsDto;
}
...
export function primitFeeFractionToDisplayBps(fraction: number): number {
...
function nextTakerDiscountPercentFromTiers(
  feeTiers: PrimitFeeInfoFeeTierDto[],
...
export function mapPrimitFeeInfoResponseToSummary(dto: PrimitFeeInfoResponseDto): FeeVipSummary {
  ...
  const tiers: FeeVipTierRow[] = sortedTiers.map((row) => ({
    ...
    makerFeeBps: primitFeeFractionToDisplayBps(parseFraction(row.maker)),
    takerFeeBps: primitFeeFractionToDisplayBps(parseFraction(row.taker)),
  }));
  ...
  const baseMakerBps = primitFeeFractionToDisplayBps(parseFraction(dto.current_maker));
  const baseTakerBps = primitFeeFractionToDisplayBps(parseFraction(dto.current_taker));
  const effMakerBps = primitFeeFractionToDisplayBps(parseFraction(dto.effective_maker));
  const effTakerBps = primitFeeFractionToDisplayBps(parseFraction(dto.effective_taker));
...
function unwrapFeeInfoPayload(json: unknown): PrimitFeeInfoResponseDto | null {
  if (!json || typeof json !== "object") return null;
  const o = json as Record<string, unknown>;
  if ("current_tier" in o && Array.isArray(o.fee_tiers)) {
    return o as unknown as PrimitFeeInfoResponseDto;
  }
  const data = o.data;
  if (data && typeof data === "object" && "current_tier" in data && Array.isArray((data as Record<string, unknown>).fee_tiers)) {
    return data as unknown as PrimitFeeInfoResponseDto;
  }
  return null;
}

/**
 * 拉取 Primit 官方 `GET /api/v1/account/fee-info`，映射为 Fee VIP 页模型。
 * 可不携带 Bearer（用于未 Sign 仅展示 `fee_tiers`；由 `fetchFeeVipSummary` 再置 `user: null`）。
 * 401/403、或 HTTP 200 且 `success: false`（无 JWT 时）返回空摘要。
 */
export async function fetchPrimitFeeInfoSummary(chainId: number, jwt: string | undefined): Promise<FeeVipSummary> {
  ...
  return mapPrimitFeeInfoResponseToSummary(payload);
}
