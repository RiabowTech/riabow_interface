...
/**
 * Primit Order Submission Hook
 *
 * This hook provides a way to submit orders through the Primit backend API.
...
// Convert protocol order type to Primit API order type
...
export function usePrimitOrderSubmit(): UsePrimitOrderSubmitResult {
  ...
  const refreshBalances = useCallback(() => {
    ...
    globalMutate([`primit-balances`, chainId, address], undefined, { revalidate: true });
    globalMutate([`primit-balances`, chainId, address], undefined, { revalidate: true });
  ...
  const submitOrder = useCallback(
    async (params: PrimitOrderParams): Promise<CreateOrderResponse> => {
      ...
      // Use the domain from backend (which has correct name like "Primit")
