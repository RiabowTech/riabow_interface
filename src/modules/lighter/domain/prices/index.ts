...
import { api } from "@/modules/lighter/api/client";
...
export async function getLimitChartPricesFromStats(
  ...
  try {
    const response = await api.getCandles(chainId, symbol, { period: period as any, limit });
...
export async function getChartPricesFromStats(
  ...
  try {
    const response = await api.getCandles(chainId, symbol, {
      period: period as any,
      start: from * 1000,
    });
    if (!response.candles || response.candles.length < 1) {
      throw new Error(`not enough prices data: ${response.candles?.length}`);
    }
    return response.candles.map((c) => ({
      time: c.time,
      open: parseFloat(c.open),
