...
import { selectGasPaymentTokenAddress } from "@/modules/lighter/store/SyntheticsStateContext/selectors/settingsSelectors";
import { useSelector } from "@/modules/lighter/store/SyntheticsStateContext/utils";
import type { ContractsChainId } from "config/chains";
import { getContract } from "config/contracts";
import { RELAY_SIMULATION_ORIGIN } from "config/dataStore";
import {
...
async function estimateArbitraryGasLimit({
  ...
  const gasLimit = await fallbackCustomError(
    async () =>
      provider
        .estimateGas({
          from: RELAY_SIMULATION_ORIGIN as Address,
...
export function getArbitraryRelayParamsAndPayload({
  ...
  const subaccountValidations =
    ...
    getSubaccountValidations({
      requiredActions: 1,
      subaccount,
      subaccountRouterAddress: getOrderRelayRouterAddress(chainId, true, isTradingAccount),
    });
...
export function useArbitraryRelayParamsAndPayload({
  ...
  const subaccount = useSelector(
    isTradingAccount ? selectSubaccountForMultichainAction : selectSubaccountForSettlementChainAction
  );
    ...
    async ({ params: p }) => {
      ...
      try {
        const expressParams = await estimateExpressParams({
          chainId,
          isTradingAccount: p.isTradingAccount,
