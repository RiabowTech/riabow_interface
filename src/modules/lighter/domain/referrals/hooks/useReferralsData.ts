    ...
    async function getChainReferralData(chainId: number): Promise<ReferralsStats | undefined> {
      ...
      return getReferralsGraphClient(chainId)
        ...
        .then(async (res) => {
          ...
          const referralCodeRows = data.referralCodes ?? [];
          const affiliateStatsRows = data.affiliateStats ?? [];
          ...
          const referralCodes = referralCodeRows.map((e) => e.code);
          ...
          const affiliateReferralCodesStats: ReferralCodeStats[] = affiliateStatsRows
            .filter((e) => {
              return referralCodeRows.some((c) => c.code === e.referralCode);
            })
            .map(
              (e): ReferralCodeStats => ({
          ...
          const affiliateTotalStats: AffiliateTotalStats = affiliateStatsRows.reduce(
          ...
          const traderReferralTotalStats: TraderReferralTotalStats = data.referralTotalStats
            ? {
                volume: BigInt(data.referralTotalStats.volume)!,
                discountUsd: BigInt(data.referralTotalStats.discountUsd)!,
                v1Data: {
                  volume: BigInt(data.referralTotalStats.v1Data.volume)!,
                  discountUsd: BigInt(data.referralTotalStats.v1Data.discountUsd)!,
                },
                v2Data: {
                  volume: BigInt(data.referralTotalStats.v2Data.volume)!,
                  discountUsd: BigInt(data.referralTotalStats.v2Data.discountUsd)!,
                },
              }
            : {
                volume: BN_ZERO,
          ...
          return {
            ...
            affiliateTierInfo: data.affiliateTierInfo,
            affiliateTotalStats,
            traderReferralTotalStats,
            codes: referralCodeRows.map((e) => decodeReferralCode(e.code)),
          } as ReferralsStats;
        });
    }
