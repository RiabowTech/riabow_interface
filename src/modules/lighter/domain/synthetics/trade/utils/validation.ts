...
import { ExternalSwapQuote, NextPositionValues, SwapPathStats, TradeFees, TriggerThresholdType } from "sdk/types/trade";
...
export function getIncreaseError(p: {
  ...
  // For trade route, prioritize "Enter an amount" over "Select a market"
  // In trade route, always show "Enter an amount" when market is not selected
  const hasNoMarket = !marketInfo || !indexToken;
  
  if (hasNoMarket) {
    // In trade route, show "Enter an amount" instead of "Select a market"
    return [t`Enter an amount`];
  }
  ...
  if (nextLeverageWithoutPnl !== undefined && nextLeverageWithoutPnl > maxAllowedLeverage) {
    return [t`Max leverage: ${Math.round(maxAllowedLeverage / BASIS_POINTS_DIVISOR)}x`];
  }
...
export function getIsMaxLeverageExceeded(
  ...
  // 防止除以零：如果 minCollateralFactor 为 0，返回 false（不视为超过最大杠杆）
