...
export function getGasPaymentValidations({
  ...
  const tokenBalance = isTradingAccount ? gasPaymentToken.tradingAccountBalance : gasPaymentToken.walletBalance;
  const isOutGasTokenBalance = tokenBalance === undefined || totalGasPaymentTokenAmount > tokenBalance;

  const needGasPaymentTokenApproval = isTradingAccount
    ? false
...
export async function buildAndSignExpressBatchOrderTxn({
  ...
  const relayRouterAddress = getOrderRelayRouterAddress(chainId, subaccount !== undefined, isTradingAccount);
