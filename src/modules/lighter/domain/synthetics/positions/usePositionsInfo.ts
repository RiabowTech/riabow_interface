...
export function usePositionsInfoRequest(
  ...
  return useMemo(() => {
    ...
    const positionsInfoData = Object.keys(positionsData).reduce((acc: PositionsInfoData, positionKey: string) => {
      ...
      // Try marketsData first, fallback to marketInfo for synthetic markets (trade mode)
      const market = getByKey(marketsData, position.marketAddress) || marketInfo;

      // Get token addresses from market (or marketInfo for synthetic markets)
      const indexToken = market
        ? getByKey(tokensData, convertTokenAddress(chainId, market.indexTokenAddress, "native")) ||
          (marketInfo as MarketInfo | undefined)?.indexToken // Use indexToken from marketInfo for synthetic markets
        : undefined;
      const longToken = getByKey(tokensData, market?.longTokenAddress) ||
        (marketInfo as MarketInfo | undefined)?.longToken;
      const shortToken = getByKey(tokensData, market?.shortTokenAddress) ||
        (marketInfo as MarketInfo | undefined)?.shortToken;
      ...
      if (!market || !indexToken || !longToken || !shortToken || !pnlToken || !collateralToken) {
        // Silently skip positions that can't be parsed - this is expected when:
        // 1. Market info hasn't loaded yet (temporary state)
        // 2. Market is not in the supported markets list (normal for API positions)
        if (process.env.NODE_ENV === "development") {
          // eslint-disable-next-line no-console
          console.debug(`Position parsing skipped (market info not available)`, {
            positionKey: position.key,
            marketAddress: position.marketAddress,
            hasMarket: !!market,
            hasIndexToken: !!indexToken,
            hasLongToken: !!longToken,
            hasShortToken: !!shortToken,
            hasPnlToken: !!pnlToken,
            hasCollateralToken: !!collateralToken,
          });
        }
        return acc;
      }
      ...
      // Use entryPrice from API if available (for API positions), otherwise calculate it
      const entryPrice = position.entryPrice !== undefined
        ? position.entryPrice
        : getEntryPrice({
      ...
      // In trade mode, collateralAmount is already in USDT (6 decimals), so we need to convert it to USD (30 decimals)
      // collateralAmount is stored with 6 decimals (USDT precision), but we need USD with 30 decimals
      // So: collateralUsd = collateralAmount * 10^(30-6) = collateralAmount * 10^24
      const isTradeMode = position.originalPositionId !== undefined;
      const collateralUsd = isTradeMode
        ? position.collateralAmount * BigInt(10 ** 24) // Convert from 6 decimals (USDT) to 30 decimals (USD)
        : convertToUsd(position.collateralAmount, collateralToken.decimals, collateralMinPrice)!;
      ...
      // In trade mode, use API returned PNL directly instead of recalculating
      // Recalculating can cause precision issues or incorrect values
      const pnl = isTradeMode
        ? position.pnl
        : marketInfo
          ? getPositionPnlUsd({
      ...
      // In trade mode, use backend's stored leverage directly to avoid
      // fee-deducted collateral causing inflated leverage display
      const leverage = isTradeMode && position.originalLeverage
        ? position.originalLeverage
        : getLeverage({
            sizeInUsd: position.sizeInUsd,
            collateralUsd: collateralUsd,
      ...
      const leverageWithPnl = isTradeMode && position.originalLeverage
        ? position.originalLeverage
        : getLeverage({
            sizeInUsd: position.sizeInUsd,
            collateralUsd: collateralUsd,
      ...
      const leverageWithoutPnl = isTradeMode && position.originalLeverage
        ? position.originalLeverage
        : getLeverage({
            sizeInUsd: position.sizeInUsd,
            collateralUsd: collateralUsd,
      ...
      // Use liquidationPrice from API if available (for API positions), otherwise calculate it
      const liquidationPrice = position.liquidationPrice !== undefined
