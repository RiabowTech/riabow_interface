...
export function sortPositionOrders(
  ...
  // Check if we're in API trading mode (API orders)
  const isTradeMode = shouldUseApiOrders();
  
  return orders.sort((a, b) => {
    // In API trading mode, sort by time (newest first)
    if (isTradeMode) {
      // Use updatedAtTime if available, otherwise use validFromTime (created time)
      const timeA = Number(a.updatedAtTime || a.validFromTime || 0n);
      const timeB = Number(b.updatedAtTime || b.validFromTime || 0n);
      // Sort descending (newest first)
      return timeB - timeA;
    }
    
    // Original sorting logic for non-API trading mode
    if (tokenSortOrder) {
