...
export function useGlvMarketsInfo(
  ...
  const glvReaderAddress = enabled ? tryGetContract(chainId, "GlvReader") : undefined;
  const canRequestGlvMarkets = Boolean(enabled && dataStoreAddress && glvReaderAddress);

  const { data: glvList, isLoading: isLoadingGlvs } = useMulticall<GlvsRequestConfig, GlvList>(
    chainId,
    "useGlvTokenMarkets",
    {
      key: canRequestGlvMarkets ? ["glvMarkets", chainId] : null,
      request: () => {
        return {
          glvs: {
  ...
  const shouldRequest = canRequestGlvMarkets && marketsInfoData && tokensData && glvs && glvs.length > 0;
