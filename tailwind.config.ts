...
import { generateColorConfig } from "./src/shared/lib/generateColorConfig";
...
function customUtilsPlugin({ addUtilities, addVariant, addComponents }: any) {
  ...
  addVariant("app-hover", [`@media (hover: hover) {&:hover}`, `@media (hover: none) {&:active}`]);
  addVariant("group-app-hover", [
  ...
  addVariant("not-group-app-hover", [
  ...
  addComponents({
    ".app-hover-gradient": {
      "@apply app-hover:bg-gradient-to-r app-hover:from-[#23263B] app-hover:to-[#16182E]": {},
    },
    ".app-hover-gradient-to-l": {
      "@apply app-hover:bg-gradient-to-l app-hover:from-[#23263B] app-hover:to-[#16182E]": {},
    },
  });
}
...
const config: Config = {
  ...
  theme: {
    ...
    extend: {
      ...
      fontFamily: {
        chakraPetch: ['"Chakra Petch"', "system-ui", "-apple-system", "BlinkMacSystemFont", '"Segoe UI"', "sans-serif"],
      },
    },
  },
